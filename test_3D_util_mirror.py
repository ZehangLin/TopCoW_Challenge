...
def normalized(image):
    ...
    # mean = 202.65733
    # sd = 100.49714
    # percentile_99_5 = 675.3030120849586
    # percentile_00_5 = 70.99015045166016

    # mean = 196.45638
    # sd = 93.75204
    # percentile_99_5 = 616.0048217773436
    # percentile_00_5 = 69.9997787475586

    mean = 1164.9766845703125
    sd = 8066.24658203125
    percentile_99_5 = 115.00224304199219
    percentile_00_5 = 76748.4765625
...
def normalized2(image):
    ...
    # percentile_99_5 = 675.3030120849586
    # percentile_00_5 = 70.99015045166016
    # mean = 196.0216522216797
    # sd = 93.0036849975586
    # percentile_99_5 = 612.9794311523438
    # percentile_00_5 = 72.99897766113281
...
def location(data_array):
    mask_voxel_coords = np.where(data_array == 1)
    ...
    for w in [100]:
        ...
        if minzidx < 0:
            minzidx = 0
        if maxzidx > data_array.shape[0] - 1:
            maxzidx = data_array.shape[0] - 1
        minxidx = int(center_x - 0.5 * diameter_x * w / 100)
        maxxidx = int(center_x + 0.5 * diameter_x * w / 100)
        if minxidx < 0:
            minxidx = 0
        if maxxidx > data_array.shape[1] - 1:
            maxxidx = data_array.shape[1] - 1
        minyidx = int(center_y - 0.5 * diameter_y * w / 100)
        maxyidx = int(center_y + 0.5 * diameter_y * w / 100)
        if minyidx < 0:
            minyidx = 0
        if maxyidx > data_array.shape[2] - 1:
            maxyidx = data_array.shape[2] - 1
        bbox = [[minzidx, maxzidx], [minxidx, maxxidx], [minyidx, maxyidx]]
...
def test_all_case_without_score(net,model_name,base_dir,base_dir2, num_classes=14, patch_size=(64, 128, 128), stride_xy=32, stride_z=24, json_path = None, test_save_path=None, TTA_flag = False):
    print("Testing begin")
    path = os.listdir(base_dir)
    _case_results = []
    for image_path in path:
    ...
    #     image = Resample(image_1, [0.70001221, 0.43164062, 0.43164062], False)
        origin = image.GetOrigin()
        spacing = image.GetSpacing()
        direction = image.GetDirection()
        ...
        if model_name == 'nnUNet' or model_name == 'nnUNet_2':
        ...
        pred_itk_1 = connected_domain_filter(prediction)
        prediction = sitk.GetArrayFromImage(pred_itk_1)
        roi_array = image_copy[location(prediction)]
        ...
        roi_save_path = r'./output/roi'
        if not os.path.exists(roi_save_path):
            os.mkdir(roi_save_path)

        sitk.WriteImage(roi, roi_save_path +
                        "/{}".format(image_path.replace('_0000', '')))

        pred_itk = sitk.GetImageFromArray(prediction.astype(np.uint8))
        pred_itk.SetOrigin(origin)
        pred_itk.SetSpacing(spacing)
        pred_itk.SetDirection(direction)
        # pred_itk = Resample(pred_itk, image_1.GetSpacing(), True, image_1.GetSize())

        sitk.WriteImage(pred_itk, test_save_path +
                        "/{}".format(image_path.replace('_0000','')))
        print('Saving',test_save_path +"/{}".format(image_path.replace('_0000','')))
...
def test_all_case_without_score_2(net, model_name, base_dir, num_classes=14, patch_size=(64, 128, 128), stride_xy=32,
                                stride_z=24, json_path=None, test_save_path=None, TTA_flag=False):
    print("Testing begin")
    path = os.listdir(base_dir)
    _case_results = []
    for image_path in path:
        ...
        mask = sitk.ReadImage(os.path.join(r'/output/images/cow-binary-segmentation', image_path))
        # mask = Resample(mask_1, [0.70001221, 0.43164062, 0.43164062], True)
        mask_array = sitk.GetArrayFromImage(mask)



        pred_all_array = np.zeros_like(mask_array)
        #     if max(np.unique(sitk.GetArrayFromImage(image)) )> 256:
        #         print('Doning Preprocessing')
        #         image = Normalize(image, -750, 3000)
        #     image.SetSpacing((1.0,1.0,1.0))
        ...
        print(prediction.shape,location((mask_array)),pred_all_array.shape)
        pred_all_array[location((mask_array))] = prediction
        pred_itk = sitk.GetImageFromArray(pred_all_array.astype(np.uint8))
        pred_itk.SetOrigin(origin)
        pred_itk.SetSpacing(spacing)
        pred_itk.SetDirection(direction)
        # pred_itk = Resample(pred_itk, mask_1.GetSpacing(), True, mask_1.GetSize())
        sitk.WriteImage(pred_itk, test_save_path +
                        "/{}".format(image_path.replace('_0000', '')))
        _case_results.append(process_case(image_path))
    save(json_path, _case_results)
