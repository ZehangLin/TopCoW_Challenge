...
from test_3D_util_mirror import test_all_case_without_score,test_all_case_without_score_2
# from test_3D_util_rt import test_all_case_without_score

parser = argparse.ArgumentParser()
parser.add_argument('--root_path2', type=str,
                    default='./input/images/head-ct-angio', help='Name of Experiment')

parser.add_argument('--root_path', type=str,
                    default='./input/images/head-mr-angio', help='Name of Experiment')
...
def Inference(FLAGS):
    num_classes = 2
    test_save_path = "./output/images/cow-binary-segmentation"
    json_path = r'./output/results.json'
    if os.path.exists(test_save_path):
        shutil.rmtree(test_save_path)
    if os.path.exists(r'/output/roi'):
        shutil.rmtree(r'/output/roi')
    if not os.path.exists(test_save_path):
        os.makedirs(test_save_path)
    if not os.path.exists(r'/output/roi'):
        os.makedirs(r'/output/roi')
    net = net_factory_3d(net_type=FLAGS.model1, in_chns=1, class_num=num_classes).cuda()
    # net = VNet(n_classes=num_classes, in_channels=1).cuda()
    # save_mode_path = os.path.join(
    #     snapshot_path, '{}_best_model.pth'.format(FLAGS.model))
    if FLAGS.model1 == 'nnUNet':
        ...
        save_mode_path = r'./weight/model_best_loc_mr.model'
    ...
    # avg_metric = test_all_case_without_score(net, model_name=FLAGS.model1, base_dir=FLAGS.root_path,base_dir2=FLAGS.root_path2,
    #                                          num_classes=num_classes,
    #                                          patch_size=(96, 192, 128), stride_xy=64, stride_z=64, json_path=json_path,
    #                                          test_save_path=test_save_path, TTA_flag=FLAGS.TTA)
    # avg_metric = test_all_case_without_score(net, model_name=FLAGS.model1, base_dir=FLAGS.root_path,
    #                                          base_dir2=FLAGS.root_path2,
    #                                          num_classes=num_classes,
    #                                          patch_size=(64, 192, 160), stride_xy=64, stride_z=64, json_path=json_path,
    #                                          test_save_path=test_save_path, TTA_flag=FLAGS.TTA)

    num_classes_2 = 16
    test_save_path_2 = "./output/images/cow-multiclass-segmentation"
    json_path = r'./output/results.json'
    ...
    net2 = net_factory_3d(net_type=FLAGS.model2, in_chns=1, class_num=num_classes_2).cuda()
    if FLAGS.model2 == 'nnUNetv2':
        save_mode_path = r'./weight/checkpoint_best.pth'
        net2.load_state_dict(torch.load(save_mode_path)['network_weights'])

    print("init weight from {}".format(save_mode_path))

    net2.eval()

    avg_metric = test_all_case_without_score(net2, model_name=FLAGS.model2, base_dir=FLAGS.root_path,
                                             base_dir2=FLAGS.root_path2,
                                             num_classes=num_classes_2,
                                             patch_size=(64, 160, 192), stride_xy=64, stride_z=64, json_path=json_path,
                                             test_save_path=test_save_path_2, TTA_flag=FLAGS.TTA)
